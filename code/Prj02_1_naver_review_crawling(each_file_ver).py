...
def crawler(year, list_start, list_end, review_start, review_step):
    ...
    for page in range(list_start, list_end + 1):
        if not os.path.isfile(f"../crawling_data/reviews_{year}_{page}.csv"):
            ...
            # df_reviews = pd.concat([df_reviews, df_page])
    driver.close()
    # df_reviews.to_csv(f"../crawling_data/reviews_{year}_{list_start}-{list_end}.csv", index=False)
...
if __name__ == "__main__":
    processes = 4  # 코어 수
    total_list = 53  # 연도별 크롤링할 페이지 수 / 총 영화 수는 대략 total_list * 10
    ...
    pool.close()
    pool.join()
    filelist = glob("../crawling_data/reviews_{year}_*.csv")
    print(len(filelist))
